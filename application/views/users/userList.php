    ...
    <button type="button" class="btn btn-default" data-dismiss="modal">Tidak</button>
          ...
          <?php endif; ?>
                ...
                <div class="card-body">
                    ...
                    <a href="<?php echo base_url('users/add'); ?>" class="btn btn-primary btn-border" style="float: right;">+ User</a>
...
$(document).ready(function() {
  ...
  "ajax": {
                        "url": '<?php echo base_url()?>users/get_json',
                        "type": "POST",
                                        "dataType": "JSON",
                        },
                        "columns": [
...
function bulk_delete()
{
    ...
    if(list_id.length > 0)
    {
        if(confirm('Anda yakin menghapus '+list_id.length+' data?'))
        {
            $.ajax({
                type: "POST",
                data: {username:list_id},
                url: "<?php echo site_url('users/bulk_delete')?>",
...
<script type="text/javascript" language="JavaScript">
 ...
 $('#modal-konfirmasi').on('show.bs.modal', function (event) {
  ...
  modal.find('#hapus-true').attr("href","users/delete/"+id);
